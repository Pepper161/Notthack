...
class _AuditorScreenState extends State<AuditorScreen> {
  ...
  Widget _buildSolanaBanner(SolanaStatus status) {
    ...
    final color = isLive ? const Color(0xFF9945FF) : Colors.grey;
    return Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  isLive
                      ? 'Solana ${status.cluster ?? "devnet"} — LIVE'
                      : 'Solana — offline',
          ...
          if (status.wallet != null)
            IconButton(
              ...
              onPressed: () => _openExplorer(
                  'https://explorer.solana.com/address/${status.wallet}?cluster=${status.cluster ?? "devnet"}'),
            ),
        ],
      ),
    );
